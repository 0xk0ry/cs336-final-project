...
class CIRPlus(nn.Module):
    ...
    def load_ckpt(self, model_path, is_origin=False):
        saved_state_dict = torch.load(model_path, map_location=self.device, weights_only=True)
        if is_origin:
    ...
    def load_combiner(self, combining_function='sum', combiner_path='', projection_dim=640 * 4, hidden_dim=640 * 8):
        if combining_function == 'sum':
            ...
            if combiner_path:
                saved_state_dict = torch.load(combiner_path, map_location=device, weights_only=True)
